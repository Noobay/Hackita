...
def set_TARGET_HOST(new_value):
    app.print_line('TARGET_HOST= "{}"'.format(new_value))
...
def do_methods(line):
    ...
    if not Aurl:
        app.print_line("methods: must specify a domain on the command line or in 'TARGET_HOST' option")
        return

    conn = httplib.HTTPConnection(Aurl)
    conn.request('OPTIONS', '/')
    response = conn.getresponse()
    app.print_line(response.getheader('allow'))
